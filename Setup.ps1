...
function Initialize-EnvFile {
    ...
    Write-Host ""
    Write-Host "  No credentials needed here." -ForegroundColor Green
    Write-Host "  Your admin account and API keys are configured in the" -ForegroundColor Gray
    Write-Host "  web console on first visit -- never written to disk." -ForegroundColor Gray
    Write-Host ""
...
# This file contains only infrastructure config (ports, bind addresses).
...
# encrypted database. SECRET_KEY is moved to the Windows DPAPI credential
# store on first startup and this file is no longer sensitive after that.
...
function Start-Install {
    ...
    Write-Host "  1. Open http://localhost:$WebPort"
    Write-Host "  2. The setup wizard will ask for your admin username and password"
    Write-Host "  3. Optionally enter your Anthropic API key during setup"
    Write-Host "  4. Point your UDM syslog to this PC on port $SyslogPort"
    Write-Host ""
    Write-Host "  Credentials are stored directly in the encrypted database." -ForegroundColor Gray
    Write-Host "  They are never written to .env or any config file." -ForegroundColor Gray
    Wait-Enter
}
